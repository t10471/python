...
DIR = 'dir'
METHOD = 'method'
MIN = {TERM: 'Y', DIR: 'min', METHOD: 'get_min'}
DAY = {TERM: 'Y', DIR: 'day', METHOD: 'get_day'}

class Google(object):

    def __init__(self):
        url = 'http://www.google.com/finance/getprices?p={0}&f=d,h,o,l,c,v&i={1}&x={2}&q={3}'
        self.getter = getter.Getter(url)
        self.parser = parser.Parser()
        self.register = register.Register()

    def run(self, term_type, span):
        span = str(span)
        ...
        for code, symbol in in_data:
            ...
            data = self.parser.parse(method(str(span), term_type[TERM], code, symbol))
            if len(data) != 0:
